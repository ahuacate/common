...
# Proxmox Version Check ($MinV is the minimum chk version)
...
function load_module() {
    if ! $(lsmod | grep -Fq $1); then
        modprobe $1 &> /dev/null || die "Failed to load '$1' module."
    fi
    MODULES_PATH=/etc/modules
    if ! $(grep -Fxq "$1" $MODULES_PATH); then
        echo "$1" >> $MODULES_PATH || die "Failed to add '$1' module to load at boot."
    fi
}
# Installer cleanup
...
function pve_version_status() {
    # Function retrieves Proxmox and Kernel versions and informs the user
    # if a upgrade is recommended.
    # This func does not require prerequisite 'pvesource_bash_defaults.sh
    # This func is used in 'pve_repo_loader' by default.
    #
    # Parameters:
    #   None
    #
...
function input_username_val() {
    # Script prompts user to set a username. Requires user input.
    # Validates username and if accepted sets variable $USERNAME
    # If input fail user is prompted to try again.
    #
    # Parameters:
    #   None
    #
...
function input_emailaddress_val() {
    # Script prompts user to set a email address. Requires user input.
    # Validates email and if accepted sets variable $USER_EMAIL
    # If input fail user is prompted to try again.
    #
    # Parameters:
    #   None
...
# Input a $USER_PWD with validation
function input_userpwd_val() {
    # Script prompts user to set a password. Requires user input.
    # Validates pwd and if accepted sets password variable $USER_PWD
    # If pwd fail user is prompted to try again.
    #
    # Parameters:
    #   None
    #
    ...
    # Install libcrack2
    if [[ ! $(dpkg -s libcrack2 2>/dev/null) ]]; then
...
function make_userpwd() {
    # Generates a password and sets variable $USER_PWD
    #
    # Parameters:
    #   None
    #
    ...
    # Check for SW
    if [[ ! $(dpkg -s makepasswd 2>/dev/null) ]]; then
...
# PCT list
function pct_list() {
    # Creates a array list of hosts CTID
    # Example of output
    #       VMID,Status,Lock,Name
    #       100,stopped,,geterr
    #       101,running,,vidcoderr
    #       102,running,,kodirsync
    #       104,stopped,,tailscale
    #       105,running,,tdarr
    #
    # Parameters:
    #   Requires perl
    ...
    # Usage:
    #   pct_LIST=( $(pct list) )
    #   pct_LIST=( $(pct list | awk 'NR > 1 { OFS = ":"; print $NF,$1 }') )  # Create menu selection list
...
function pct_stop_systemctl() {
    # Stops a system.d service
    #
    # Parameters:
    #   None
    #
    # Usage:
    #   pct_stop_systemctl "name.service"
...
function pct_start_systemctl() {
    # Start a system.d service
    #
    # Parameters:
    #   None
    #
    # Usage:
    #   pct_start_systemctl "name.service"
...
function pct_restart_systemctl() {
    # Restarts a system.d service
    #
    # Parameters:
    #   None
    #
    # Usage:
    #   pct_restart_systemctl "name.service"
    ...
    elif [ "$(systemctl is-active $service_name)" = 'active' ]; then
        # Stop service
...
function pct_check_systemctl() {
    # Checks a system.d service status
    #
    # Parameters:
    #   None
    #
    # Usage:
    #   pct_check_systemctl "name.service"

    # Set other variables
    local service_name="$1"
    local FAIL_MSG='Systemctl '${service_name}' has failed. Reason unknown.\nExiting installation script in 2 second.'
    local i=0
...
function usb_reset(){
    ...
    # Set other variables
    local base="/sys/bus/pci/drivers"
    local sleep_secs="1"
...
function input_dirname_val() {
    # Script prompts user to set dir name. Requires user input.
    # Validates input and if accepted sets variable $DIR_NAME
    # If input fails user is prompted to try again.
    #
    # Parameters:
    #   None
    #
...
function makeselect_input1 () {
    # Part of the special menu selection
    #
    # Parameters:
    #   None
    #
...
function edit_config_value() {
    ...
    if egrep -q "^(#)?(\s)?$key(\s)?=" "$config_file"; then
        ...
        if [ -z "$comment" ]; then
            sed -i "s/^\(\s*\)#\{0,1\}\(\s*\)$key\(\s*\)=.*/$key=\"$value\" $existing_comment/" "$config_file"  # If no comment is provided, use the existing comment line
        else
            sed -i "s/^\(\s*\)#\{0,1\}\(\s*\)$key\(\s*\)=.*/$key=\"$value\" # $comment/" "$config_file"  # If a comment is provided, use the new comment line
        fi
    else
        # Add the key-value pair to the end of the config file
        if [ -z "$comment" ]; then
            echo "$key=\"$value\"" >> "$config_file"  # If no comment is provided, don't include a comment line
        else
            echo "$key=\"$value\" # $comment" >> "$config_file"  # If a comment is provided, include the comment line
        fi
    fi
}
...
edit_json_value() {
    ...
    #
    # Usage:
    #   edit_json_value config.json name "Jane"
...
function check_smtp_status() {
    # Check SMTP status if var 'ahuacate_smtp' exists in '/etc/postfix/main.cf'
    # This checks if SMTP has been configured using Ahuacate scripts
    # Sets $SMTP_STATUS
    ...
    # Set other variables
    local var='ahuacate_smtp'
    local file='/etc/postfix/main.cf'

    if [ -f $file ] && [ "$(systemctl is-active --quiet postfix; echo $?)" = 0 ]; then
        SMTP_STATUS=$(grep --color=never -Po "^${var}=\K.*" "${file}" || true)  # Set SMTP active
    else
        SMTP_STATUS=0  # Set SMTP inactive
    fi
}
...
function check_host_subid() {
    # For PVE hosts only. Checks '/etc/subgid' and '/etc/subuid' entries exist.
    #
    # Parameters:
    #   None
    #
    ...
    # Set other variables
    local subgid_root_entry_1="root:65604:100"
    local subgid_root_entry_2="root:100:1"
    local subuid_root_entry_1="root:1605:1"
    local subuid_root_entry_2="root:1606:1"
    local subuid_root_entry_3="root:1607:1"

    # Check if all the subgid/subuid entries exist
    if ! grep -qF "$subgid_root_entry_1" /etc/subgid || ! grep -qF "$subgid_root_entry_2" /etc/subgid || ! grep -qF "$subuid_root_entry_1" /etc/subuid || ! grep -qF "$subuid_root_entry_2" /etc/subuid || ! grep -qF "$subuid_root_entry_3" /etc/subuid; then
...
# Check Install CT SW status (active or abort script)
function pct_check_systemctl() {
    # Checks the status of a system.d service
    #
    # Parameters:
    #   None
    #
    # Usage:
    #   check_systemctl_sw "service.service"
...
# General display message functions
if [[ ! $(dpkg -s boxes 2> /dev/null) ]]; then
  apt-get install -y boxes > /dev/null  # Required by msg display func msg_box
fi
function msg() {
    local TEXT="$1"
...
function indent2() {
    sed "s/^/  /"  # Use with pipe echo 'sample' | indent2
}
...
# IP validate
function valid_ip() {
    # Validate a IP address. Check IPv4 and IPv6
    # Returns '0' or '1'
    #
    # Parameters:
    #   None
...
# Position terminal top/left of screen
printf '\e[3;0;3t'

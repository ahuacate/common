...
then
  ...
  if [ "$(pct exec $CTID -- dpkg -s libuser >/dev/null 2>&1; echo $?)" = 0 ]
  then
...
then
  # Check user list (CT)
  user_check_LIST=()
  while IFS='' read -r grp
  do
    if [[ $(pct exec $CTID -- getent group ${grp}) ]]
    then
...
then
  ...
  elif [[ ! $(df -h | awk 'NR>1 { print $1, "mounted on", $NF }' | grep "/mnt/pve/.*backup") ]]
  then
    msg "--  BACKUP LOCATION OF SSH PUBLIC KEY FILES
...
do
  ...
  then
    # Check for .ssh folder
    if [ $(pct exec $CTID -- bash -c "[[ ! -d "$ssh_home_dir" ]] && echo 0") ]
    then
      pct exec $CTID -- mkdir -p $ssh_home_dir/.ssh
      pct exec $CTID -- chmod 700 $ssh_home_dir/.ssh
    fi
    # Check for .ssh/authorized_keys file
    if [ $(pct exec $CTID -- bash -c "[[ ! -f "$ssh_home_dir/.ssh/authorized_keys" ]] && echo 0") ];
    then

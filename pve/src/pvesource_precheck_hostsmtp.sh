...
then
  ...
  # Pre-check picasso566 ES SMTP
  if [ "$(grep --color=never -Po "^${var}=\K.*" "${file}" || true)" = 1 ]
  then
    precheck_msg1+=( "picasso566 SMTP Easy Script status:pass" )
  else
    precheck_msg1+=( "picasso566 SMTP Easy Script status:not installed (not required for manual configs)" )
  fi
